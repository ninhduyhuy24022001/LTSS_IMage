...
{
    // Vị trí đầu tiên của block chứa thread
	int i = blockIdx.x * blockDim.x;

    for (int stride = 1; stride < blockDim.x; stride *= 2)
    {
        // Bước nhảy 2 để bỏ qua những thread không thực hiện tính toán
        int index = stride * 2 * threadIdx.x;
         
        if(index < blockDim.x)
            in[index + i] += in[index + i + stride];
...
__global__ void reduceBlksKernel3(int * in, int n, int * out)
{
    ...
    // stride >>=1 dịch qua phải 1 bit đối với số mủ 2 là căn 2.
        ...
        bool useDevice=false, dim3 blockSize=dim3(1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		int * d_in, * d_out;
		dim3 gridSize((n - 1) / blockSize.x + 1); // TODO: Compute gridSize from n and blockSize
